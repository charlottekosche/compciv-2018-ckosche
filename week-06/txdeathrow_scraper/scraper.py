...
def wrangle_inmate_data_from_tag(rowtag):
    ...
    columns = rowtag.select('td')
    inmate_dictionary = {}
    inmate_dictionary['tdcj_id'] = columns[0].text.strip()
    href = columns[1].select('a')[0].attrs['href'].strip()
    inmate_dictionary['url'] = make_absolute_url(href).strip()
    inmate_dictionary['last_name'] = columns[2].text.strip()
    inmate_dictionary['first_name'] = columns[3].text.strip()
    inmate_dictionary['birthdate'] = txdate_to_iso(columns[4].text.strip())
    inmate_dictionary['gender'] = columns[5].text.strip()
    inmate_dictionary['race'] = columns[6].text.strip()
    inmate_dictionary['date_received'] = txdate_to_iso(columns[7].text.strip())
    inmate_dictionary['county'] = columns[8].text.strip()
    inmate_dictionary['date_offense'] = txdate_to_iso(columns[9].text.strip())
    inmate_dictionary['age_at_offense'] = round(calc_years_diff(inmate_dictionary['birthdate'].strip(), inmate_dictionary['date_offense'].strip()))
    inmate_dictionary['years_before_death_row'] = calc_years_diff(inmate_dictionary['date_offense'].strip(), inmate_dictionary['date_received'].strip())

    return inmate_dictionary